...
- **FR-3**: Space-themed visual identity via design tokens, unified `FrostedPanel` / section bands, Geist display typography, and shadcn semantic CSS variables (see ADR 0013).
...
- **FR-7b**: Main nav `aria-current` follows scroll position on home (`IntersectionObserver`); URL hash syncs on scroll; `#contact` and hash clicks handled correctly (see ADR 0008).
...
- **NFR-2 Accessibility**: WCAG 2.1 AA target; muted text ≥ 4.5:1 on void; keyboard operable CTAs; `prefers-reduced-motion` respected.
- **NFR-9 Visual system** (Phase 7): Shared frosted surfaces, alternating section bands, consistent section headings; documented in [visual-principles.md](visual-principles.md) and ADR 0013.
